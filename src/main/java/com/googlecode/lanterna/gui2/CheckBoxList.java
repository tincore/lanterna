...
public class CheckBoxList<V> extends AbstractListBox<V, CheckBoxList<V>> {
    ...
    private final List<Boolean> itemStatus = new ArrayList<>();
    ...
    public CheckBoxList(Attributes attributes) {
     ...
     * @return Itself
     */
    public synchronized CheckBoxList<V> addItem(V item, boolean checkedState) {
        itemStatus.add(checkedState);
        return super.addItem(item);
    }

    @Override
    public synchronized CheckBoxList<V> addItems(List<V> items) {
        IntStream.range(0, items.size()).forEach(i -> itemStatus.add(false));
     ...
     *
     * @param selectionListener Listener to attach to this {@code CheckBoxList}
     * @return Itself
     */
    public synchronized CheckBoxList<V> addSelectionListener(SelectionListener selectionListener) {
    ...
    private void fireSelectionChanged(int index, boolean checked) {
        runOnGUIThreadIfExistsOtherwiseRunDirect(() -> {
            selectionListeners.forEach(l -> l.onItemSelected(index, checked, this));
        });
    }
     ...
     *
     * @param selectionListener Listener to remove from this {@code CheckBoxList}
     * @return Itself
     */
    public CheckBoxList<V> removeSelectionListener(SelectionListener selectionListener) {
    ...
    private void setChecked(int index, boolean checked) {
        ...
        fireSelectionChanged(index, checked);
    }
    ...
    public interface SelectionListener {
         ...
         */
        void onItemSelected(int itemIndex, boolean checked, CheckBoxList source);
    }
